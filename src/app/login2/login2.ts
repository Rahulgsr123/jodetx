...
export class Login2 implements AfterViewInit {
...
@ViewChild('leftVideo') private leftVideo?: ElementRef<HTMLVideoElement>;
...
onLeftVideoReady(): void {
  this.playLeftVideo();
}

onLeftVideoPaused(): void {
  this.playLeftVideo();
}

onLeftVideoError(event: Event): void {
  console.error('[AcquirerX] left panel video failed to load:', event);
}

private playLeftVideo(): void {
  const video = this.leftVideo?.nativeElement;
  ...
  if (playPromise !== undefined) {
    playPromise.catch((error) => {
      console.warn('[AcquirerX] video autoplay retry failed:', error);
    });
  }
}
