...
@Component({
  selector: 'app-bharat-bank',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './bharat-bank.html',
  styleUrl: './bharat-bank.scss',
})
export class BharatBank {
...
registrationToken = signal('');
  ...
  readonly pageData: AcquireXLoginData = {
    ...
    forms: {
      ...
      registerPhone: {
        title: 'Create your Bharat Bank access',
      ...
      registerPin: {
        ...
        subtitle: 'Your PIN protects every Bharat Bank session. Choose a 6 digit code.',
    ...
    validations: {
      ...
      pinInvalid: 'PIN must be 6 numeric digits.',
      ...
      registrationSuccess: 'Registration complete. Welcome to Jodetx.',
      loginSuccess: 'Authenticated. Redirecting to your console…',
    },

    links: {
      footerLeft: '© Jodetx Technologies · Jodetx Secure Access',
  ...
  // Digit-box helpers for OTP / Login PIN / Register PIN / Confirm PIN.
  readonly otpDigits = signal<string[]>(Array(6).fill(''));
  readonly pinDigits = signal<string[]>(Array(6).fill(''));
  readonly confirmPinDigits = signal<string[]>(Array(6).fill(''));

  onDigitInput(
    event: Event,
    index: number,
    type: 'otp' | 'pin' | 'confirmPin',
    formName: 'login' | 'otp' | 'pin',
    ...
    const boxes = this.getDigitBoxes(container);

    if (value && index < boxes.length - 1) {
      boxes[index + 1].focus();
    }

    this.syncDigitsToForm(type, formName, boxes);
  }
  ...
  onDigitPaste(
    ...
    const focusIndex = Math.min(Math.max(digits.length, 1), boxes.length) - 1;
    boxes[focusIndex]?.focus();

    this.syncDigitsToForm(type, formName, boxes);
  }

  private getDigitBoxes(container: HTMLElement): HTMLInputElement[] {
    return Array.from(
      container.querySelectorAll<HTMLInputElement>('.acx-digit-box')
    );
  }

  private syncDigitsToForm(
    type: 'otp' | 'pin' | 'confirmPin',
    formName: 'login' | 'otp' | 'pin',
    ...
    if (formName === 'login' && type === 'pin') {
      this.loginForm.controls.pin.setValue(value, { emitEvent: true });
      this.loginForm.controls.pin.markAsTouched();
      this.loginForm.controls.pin.updateValueAndValidity({ emitEvent: false });
      return;
    }

    if (formName === 'otp' && type === 'otp') {
      this.otpForm.controls.otp.setValue(value, { emitEvent: true });
      this.otpForm.controls.otp.markAsTouched();
      this.otpForm.controls.otp.updateValueAndValidity({ emitEvent: false });
      return;
    }

    if (formName === 'pin' && type === 'pin') {
      this.pinForm.controls.pin.setValue(value, { emitEvent: true });
      this.pinForm.controls.pin.markAsTouched();
      this.pinForm.controls.pin.updateValueAndValidity({ emitEvent: false });
      this.pinForm.updateValueAndValidity({ emitEvent: false });
      return;
    }

    if (formName === 'pin' && type === 'confirmPin') {
      this.pinForm.controls.confirmPin.setValue(value, { emitEvent: true });
      this.pinForm.controls.confirmPin.markAsTouched();
      this.pinForm.controls.confirmPin.updateValueAndValidity({ emitEvent: false });
      this.pinForm.updateValueAndValidity({ emitEvent: false });
    }
  }
}
